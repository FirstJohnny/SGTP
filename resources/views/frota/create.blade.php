...
@section('title', 'Novo Veículo - SGTP')
@section('page-title', 'Cadastrar Novo Veículo')

@section('content')
<div class="card">
    <div class="card-header">
        <span><i class="fas fa-truck"></i> Dados do Veículo</span>
        <a href="{{ route('frota.index') }}" class="btn-sm" style="background: var(--gray-500);">
            <i class="fas fa-arrow-left"></i> Voltar
        </a>
    </div>
    
    <form action="{{ route('frota.store') }}" method="POST" id="veiculoForm">
        @csrf
        ...
        <div class="row-cards" style="grid-template-columns: repeat(auto-fit, minmax(300px, 1fr));">
            <div>
                <div class="form-group">
                    ...
                    <input type="text" name="placa" class="form-control @error('placa') is-invalid @enderror" value="{{ old('placa') }}" required placeholder="Ex: LD-01-23">
                    @error('placa') <small style="color: red;">{{ $message }}</small> @enderror
                </div>
                
                <div class="form-group">
                    <label class="form-label">Chassi <span style="color: red;">*</span></label>
                    <input type="text" name="chassi" class="form-control @error('chassi') is-invalid @enderror" value="{{ old('chassi') }}" required>
            ...
            <div>
                <div class="form-group">
                    ...
                    <input type="number" name="ano_fabricado" class="form-control @error('ano_fabricado') is-invalid @enderror" value="{{ old('ano_fabricado', date('Y')) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="cor" class="form-control @error('cor') is-invalid @enderror" value="{{ old('cor') }}" required>
                    @error('cor') <small style="color: red;">{{ $message }}</small> @enderror
                </div>
                
                <div class="form-group">
                    <label class="form-label">Lotação (passageiros) <span style="color: red;">*</span></label>
                    <input type="number" name="lotacao" class="form-control @error('lotacao') is-invalid @enderror" value="{{ old('lotacao', 50) }}" required>
            ...
            <div>
                <div class="form-group">
                    ...
                    <input type="number" step="0.01" name="consumo_medio" class="form-control" value="{{ old('consumo_medio') }}">
                </div>
                
                <div class="form-group">
                    <label class="form-label">KM Atual <span style="color: red;">*</span></label>
                    <input type="number" name="km_atual" class="form-control @error('km_atual') is-invalid @enderror" value="{{ old('km_atual', 0) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="data_aquisicao" class="form-control @error('data_aquisicao') is-invalid @enderror" value="{{ old('data_aquisicao', date('Y-m-d')) }}" required>
            ...
            <div>
                <div class="form-group">
                    ...
                    <input type="date" name="seguro_validade" class="form-control @error('seguro_validade') is-invalid @enderror" value="{{ old('seguro_validade', date('Y-m-d', strtotime('+1 year'))) }}" required>
                    @error('seguro_validade') <small style="color: red;">{{ $message }}</small> @enderror
                </div>
                
                <div class="form-group">
                    <label class="form-label">Última Inspeção</label>
                    <input type="date" name="ultima_inspecao" class="form-control" value="{{ old('ultima_inspecao') }}">
                </div>
                
                <div class="form-group">
                    <label class="form-label">Próxima Inspeção</label>
                    <input type="date" name="proxima_inspecao" class="form-control" value="{{ old('proxima_inspecao') }}">
                </div>
...
<script>
    ...
    document.getElementById('veiculoForm').addEventListener('submit', function(e) {
        ...
        if(!placaRegex.test(placa.toUpperCase())) {
            e.preventDefault();
            alert('Placa inválida! Formato esperado: AAA1234');
        }
    });
</script>
@endpush